...
def main():
    ...
    use_nginx = yes_no("Include nginx reverse proxy? (No = expose frontend/backend directly)", True)
...
"""
    ...
    if use_tls and yes_no("Generate self-signed cert now with openssl?", True):
...
"""
        ...
        # No nginx: expose frontend/backend directly
        compose_content = f"""# Generated by quick_setup.py (prebuilt images, no nginx)
services:
  arpwatch:
    image: netwho/arpwatch-webui:arpwatch-latest
    container_name: arpwatch-daemon
...
2) Certificates (if TLS not generated here):
   - Self-signed: ./nginx/generate-self-signed-cert.sh (or rerun quick_setup with self-signed)
